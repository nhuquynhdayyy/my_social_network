...
# === BỔ SUNG: Hoàn thiện các Class-Based Views ===
...
# View xử lý logic thả cảm xúc (GIỮ NGUYÊN LOGIC THÔNG BÁO GỐC CỦA BẠN)
@login_required
@require_POST
def react_to_post(request, post_id):
    try:
        ...
        # === CẬP NHẬT: Logic kiểm tra quyền chặt chẽ hơn ===
        ...
        if not can_react:
            return JsonResponse({'status': 'error', 'message': 'Không có quyền thực hiện hành động này'}, status=43)

        content_type = ContentType.objects.get_for_model(Post)
        existing_reaction = Reaction.objects.filter(user=viewer, content_type=content_type, object_id=post.id).first()
        ...
        if existing_reaction:
            ...
            Reaction.objects.create(
                user=viewer, content_type=content_type, object_id=post.id, reaction_type=reaction_type
            )
            current_user_reaction = reaction_type
            # Logic tạo thông báo khi reaction lần đầu (Code gốc của bạn - Đã giữ lại)
            if viewer != author:
                Notification.objects.create(
                    recipient=author, sender=viewer, notification_type='POST_REACTION',
                    target_content_type=content_type, target_object_id=post.id
...
# === BỔ SUNG: Hàm reaction_detail và các hàm xử lý comment ===
def reaction_detail(request, pk):
    post = Post.objects.get(pk=pk)
    post_type = ContentType.objects.get_for_model(Post)
    reactions = Reaction.objects.filter(content_type=post_type, object_id=post.id)
    data = {}
...
def add_comment(request, post_id):
    ...
    if form.is_valid():
        ...
        if request.user != post.author:
            Notification.objects.create(
                recipient=post.author, sender=request.user, notification_type='POST_COMMENT',
                target_content_type=ContentType.objects.get_for_model(post), target_object_id=post.id
            )
...
def react_to_comment(request, comment_id):
    ...
    existing_reaction = Reaction.objects.filter(user=viewer, content_type=content_type, object_id=comment.id).first()
